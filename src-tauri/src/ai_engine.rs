...
impl AIEngine {
    ...
    async fn generate_openai_insights(&self, tasks: &[Task], api_key: &str) -> Result<Vec<String>> {
        let completed_tasks = tasks.iter().filter(|t| matches!(t.status, crate::models::TaskStatus::Completed)).count();
        let total_tasks = tasks.len();
        let high_priority_tasks = tasks.iter()
            .filter(|t| matches!(t.priority, crate::models::Priority::High | crate::models::Priority::Critical) 
        ...
        if total_tasks == 0 {
            insights.push("Start by adding your daily tasks to track progress effectively.".to_string());
        } else {
            let completion_rate = (completed_tasks as f32 / total_tasks as f32) * 100.0;
            
            if completion_rate >= 80.0 {
                insights.push(format!("Excellent progress! You've completed {:.0}% of your tasks. 🎉", completion_rate));
            } else if completion_rate >= 50.0 {
                insights.push(format!("Good momentum! Focus on completing the remaining {} tasks.", total_tasks - completed_tasks));
            } else if high_priority_tasks > 0 {
                insights.push(format!("{} high-priority tasks need attention. Consider tackling these first.", high_priority_tasks));
            } else {
                insights.push("Break down large tasks into smaller, manageable chunks for better progress.".to_string());
            }
        }

        if overdue_tasks > 0 {
            insights.push(format!("{} tasks are overdue. Prioritize these to get back on track.", overdue_tasks));
        }

        // Time-based insights
        let current_hour = chrono::Utc::now().hour();
        match current_hour {
            9..=11 => insights.push("Peak productivity hours: 9-11 AM. Use this time for challenging tasks.".to_string()),
            14..=16 => insights.push("Post-lunch dip is normal. Consider lighter tasks or a short break.".to_string()),
            17..=19 => insights.push("End of workday approaching. Review what you've accomplished today.".to_string()),
            _ => {}
        }

        // Ensure we have at least one insight
        if insights.is_empty() {
            insights.push("Stay focused on your goals. Small consistent progress leads to big results!".to_string());
        }
    ...
    pub async fn generate_accountability_message(&self, tasks: &[Task]) -> Result<String> {
        ...
        if let Some(api_key) = &self.openai_api_key {
            match self.generate_openai_accountability_message(completed_tasks, pending_tasks, api_key).await {
    ...
    async fn generate_openai_accountability_message(&self, completed: usize, pending: usize, api_key: &str) -> Result<String> {
        let prompt = format!(
    ...
    pub async fn analyze_productivity_patterns(&self, tasks: &[Task]) -> Result<Vec<String>> {
        ...
        if let Some((most_common_category, count)) = category_counts.iter().max_by_key(|(_, &count)| count) {
            patterns.push(format!("Most frequent category: {} ({} tasks)", most_common_category, count));
        }

        // Average completion time for completed tasks
        let completed_with_time: Vec<_> = tasks.iter()
            .filter(|t| matches!(t.status, crate::models::TaskStatus::Completed) && t.actual_time.is_some())
            .collect();

        if !completed_with_time.is_empty() {
            let avg_time: f32 = completed_with_time.iter()
                .map(|t| t.actual_time.unwrap() as f32)
                .sum::<f32>() / completed_with_time.len() as f32;
        ...
        if high_priority_count > 0 {
            let high_priority_percentage = (high_priority_count as f32 / tasks.len() as f32) * 100.0;
            patterns.push(format!("{:.0}% of tasks are high priority", high_priority_percentage));
        }
